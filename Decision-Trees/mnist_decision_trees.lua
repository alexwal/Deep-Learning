-- Builds a decision tree from MNIST dataset with ID3 Algorithm
-- Download MNIST data from: http://pjreddie.com/projects/mnist-in-csv/
...
function split(inputstr, sep)
    ...
    -- move first to last in mnist csv files
    t[i]=t[0]
    t[0]=nil
    return t
end
...
for i=1,#split(lines[1]) do
    table.insert(header, 'px' .. tostring(i))
end
local data_starts_at = 1
...
local function preprocess(data,test)
    ...
    for attr_index=1,data_dims do
        xlua.progress(attr_index, data_dims)
...
local num_trees = params.trees 
local trees = {}
for t=1,num_trees do
    print('Building tree:', t)
